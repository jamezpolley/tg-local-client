...
def build_prompt(role: str, waiting_on: str, message: str, sender: str = "") -> str:
    ...
    `sender` (e.g. "@jaypoe (id=174969502)") is injected so the classifier can
    apply sender-based rules in the role (e.g. "always ACT on messages from
    James") — without it, Haiku sees only the text and cannot tell who sent it.
    """
...
def should_act(message: str, config: TriageConfig,
    ...
    `sender` is an optional human-readable sender identity (e.g. "@jaypoe")
    injected into the prompt so the classifier can reason about who sent it.
